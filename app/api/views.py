...
async def take_book(
    ...
    users_book_info = await UsersDAO.get_user_with_books(
...
async def return_books(
    ...
    info_about_book = await HandingBookUserDAO.find_one_or_none(
        session=session, filters=HandingBookFind(book_id=book_id, user_id=user_data.id)
    )
...
async def get_book_info(
    ...
    result = await BookDAO.get_book_list(
        session=session, author_id=author_id, page=page, page_size=page_size
    )
    return result
